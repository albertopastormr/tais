...
public:
    ...
    void rango_avl(int const & k1, int const & k2, std::vector<Clave> & sol) const{
        rango_claves(this->raiz,k1,k2,sol);
    }

protected:
    void rango_claves(Link lnk, int const & k1, int const & k2, std::vector<Clave> & sol) const {
        if(lnk->iz == nullptr && lnk->dr == nullptr){
            if(lnk->cv.clave >= k1 && lnk->cv.clave <= k2)
                sol.push_back(lnk->cv.clave);
        }
        else if(lnk->iz == nullptr){
            if(lnk->cv.clave >= k1 && lnk->cv.clave <= k2)
                sol.push_back(lnk->cv.clave);
            if(lnk->cv.clave < k2)
                rango_claves(lnk->dr,k1,k2,sol);
        }
        else if(lnk->dr == nullptr){
            if(lnk->cv.clave > k1) {
                rango_claves(lnk->iz,k1, k2, sol);
            }
            if(lnk->cv.clave >= k1 && lnk->cv.clave <= k2)
                sol.push_back(lnk->cv.clave);
        }
